...
def find_all_obj(lst_, obj_, list_obj=False):
    ...
    if list_obj:
        check_type(obj_)
    res = list()
    start = find(lst_, obj_, 0, list_obj)
    while start != -1:
        res.append(start)
        start = find(lst_, obj_, start + 1, list_obj)
    return res
    
    
def replace(lst_, start_, end_, obj_):
    check_type(lst_)
    res = lst_[:start_] 
    res.append(obj_)
    res += lst_[end_ + 1:]
...
def tuple_from_list(lst_, fltr_=None, return_list=False, include_mode=False):
    if exclude_list is None:
        ...
        res = list()
        for each in lst_:
            if not(bool(each not in fltr) ^ bool(include_mode)):
                res.append(each)
    if return_list:
        return res
    else:
...
def find_list(lst_, start_symb='(', delim_symb=',', end_symb=')', start_=0, return_list=False):
    ...
    for each in delim_list:
        ...
        tmp = lst_[st: en + 1]
        try:
            _valid_list_struct(lst_, start_symb, delim_symb, end_symb)
